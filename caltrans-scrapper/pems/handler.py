...
# Local imports
from pems.settings import DATA_PATH, BASE_URL, DISTRICTS, CLEARING_HOUSE_URL, SPATIAL_TIMESERIES_URL
...
class PeMSHandler(object):
    ...
    def _convert_date_to_string(date:datetime)->str :
        ...
        if(date.day<10):
            res +='0'
        res += str(date.day) + '%02F'
        if(date.month<10):
            res +='0'
        res += str(date.month)+ '%02F'
        res += str(date.year)
        return res
                   
    def get_data(self, start_time,name,ext):
        data = list()
        locale.setlocale(locale.LC_ALL, 'en_US.UTF-8')
        for i in start_time:
            s_f = self._convert_date_to_string(i)
            # response = self._open_url(url=SPATIAL_TIMESERIES_URL.format(BASE_URL, str(int(i.timestamp())),s_f))
            # self.log.info(str(response.read()))
            self._download_file(file_name='spatial_' + name + '_' +str(i.day) +'Feb.xlsx', 
                                file_url=SPATIAL_TIMESERIES_URL.format(BASE_URL, str(int(i.timestamp())),s_f,ext),
                                save_path=None)
            # if response:
            #     data.extend(response['data'])
                # data.extend(self._collect_available_files(file_type=url['file_type'], district=url['district'],
                                                        #    year=url['year'], response=response, months=months))
        # urls=self._get_urls(start_year=start_time,end_year=end_year,)
    # def _collect_data(response):
    #     for 
        return data
    ...
    def _open_url(self, url):
        ...
        # self.log(self.browser.response().read())
    ...
    def download_files(self, start_year, end_year, districts, file_types, months, save_path=None):
        ...
        if not files_to_download.empty:
            for index, row in files_to_download.iterrows():
                success = self._download_file(file_name='sp' + str(index)+'Feb',
    ...
    def _download_file(self, file_name, file_url, save_path):
        ...
        try:
            ...
            save_path = self._create_data_directory(save_path=save_path)
            self.browser.retrieve(file_url, os.path.join(save_path, file_name))
            self.log.info('Download completed')
            return True
        except Exception as e:
            self.log.info('Error downloading {} {}'.format(file_name,e))
            return False
    ...
    def _browser_login(self):
        ...
        browser.open(BASE_URL + "/?report_form=1&dnode=Freeway&content=spatial&tab=contours&export=&fwy=10&dir=W")
